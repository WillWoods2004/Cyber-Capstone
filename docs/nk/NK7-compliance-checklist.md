...
| Data Protection | Ciphertext-only vault storage and transport | `frontend/src/crypto/crypto.ts`, `frontend/src/components/VaultPanel.tsx`, `frontend/src/pages/Dashboard.tsx`, `scripts/ops/assert-no-legacy-cloud-save.ps1` | Repo fixed; live redeploy pending |
...
- The currently deployed Amplify bundle still contains the legacy plaintext helper, so this control is not fully closed until the frontend is redeployed.
